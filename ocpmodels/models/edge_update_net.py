...
class GaussianSmearing(torch.nn.Module):
    ...
    def forward(self, dist):
        dist = dist.view(-1, 1) - (
            -self.m_min + self.coeff * self.offset.view(1, -1)
        )
...
class EdgeUpdateNet(nn.Module):
    ...
    def forward(self, z, edge_index, distances, batch=None):
        ...
        for edge_update, interaction in zip(
            self.edge_updates, self.interactions
        ):
...
@registry.register_model("edgeupdatenet")
